...
static inline float Luma(const float4& c) {
...
EnvAliasHost buildEnvAlias(const float4* texels, int W, int H) {
    ...
    if (total <= 0.0) {
        for (int i = 0; i < N; ++i) {
            out.P[i] = 1.0f / float(N);
        }
    } else {
        for (int i = 0; i < N; ++i) {
            out.P[i] = float(out.P[i] / total);
        }
    }
...
void uploadEnv(const EnvImageHost& img, const EnvAliasHost& alias, EnvironmentMap& env) {
    ...
    cudaMemcpy2DToArray(env.array, 0, 0, img.texels.data(), 
                       img.width * sizeof(float4), img.width * sizeof(float4), 
                       img.height, cudaMemcpyHostToDevice);
