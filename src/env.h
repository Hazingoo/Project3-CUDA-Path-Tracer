...
struct EnvironmentMap {
    ...
    cudaArray_t array = nullptr; // backing array for texture
    int width = 0;
    int height = 0;
    // Flattened size = W*H
    float* P = nullptr;     // device pmf per pixel, sums to 1
    float* q = nullptr;     // device alias thresholds in [0,1]
    int*   alias = nullptr; // device alias indices
};
...
struct EnvImageHost {
    int width = 0;
    int height = 0;
    std::vector<float4> texels; // row-major RGBA (A unused)
};
...
// Device-side utilities (headers only)
#ifdef __CUDACC__
#include <glm/glm.hpp>
...
__device__ __forceinline__ glm::vec3 sampleEnvAlias(const EnvironmentMap& env, float u1, float u2,
                                                    glm::vec3& wi, float& pdf)
{
    const int W = env.width, H = env.height, N = W*H;
    int i = min(int(u1 * N), N-1);
    float t = u2;
    int pick = (t < env.q[i]) ? i : env.alias[i];

    int x = pick % W, y = pick / W;
...
__device__ __forceinline__ float envPdfAlias(const EnvironmentMap& env, const glm::vec3& dir){
    ...
    return (env.P[pick] * float(env.width*env.height)) /
           (2.f*CUDART_PI_F*CUDART_PI_F * sinT);
}

__device__ inline void dirToUV(const glm::vec3& d, float& u, float& v){
    glm::vec3 n = glm::normalize(d);
    float phi = atan2f(n.z, n.x); if (phi < 0) phi += 2.f*CUDART_PI_F;
    float theta = acosf(fminf(fmaxf(n.y,-1.f),1.f));
    u = phi/(2.f*CUDART_PI_F); v = theta/CUDART_PI_F;
}

__device__ inline glm::vec3 uvToDir(float u, float v){
    float phi = u*2.f*CUDART_PI_F, theta = v*CUDART_PI_F;
